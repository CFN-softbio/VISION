...
def compare_csv_results(csv_path):
    with open(csv_path, 'r') as csvfile:
        ...
        for row_id, row in enumerate(reader):
            ...
            codebleu_scores = json.loads(row['codebleu_scores'])
            ...
            comparisons = [
                {
                    ...
                    'codebleu_score': codebleu_scores[i],
                }
                for i, expected_code in enumerate(expected_codes)
            ]

            # Sort by averaged normalized Levenshtein distance score and CodeBLEU score
            comparisons.sort(key=lambda x: ((1 - x['normalized_levenshtein_distance']) + x['codebleu_score']['codebleu']) / 2, reverse=True)
...
def main():
    ...
    if args.csv_path:
        ...
        if not subdirs:
            raise FileNotFoundError("No subdirectories found in the results/op_cog directory.")
