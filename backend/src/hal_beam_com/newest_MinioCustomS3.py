...
def initialize_models():
    ...
    # Get unique text models from configuration
    text_models = set(config["text"].values())
    
    # Get audio models with their finetuning status
...
class CustomS3(Base):
    ...
    def __init__(self,
        ...
        # If it was not defined in the env variables, throw an error
        if secret_key is None:
            raise EnvironmentError("SECRET_S3_KEY environment variable is not set.")
    ...
    def clear(self, name=None):
        ...
        received = '{}/{}-received.npy'.format(self.save_dir, name)
        ...
        sent = '{}/{}-sent.npy'.format(self.save_dir, name)
...
# TODO: This should not be uploaded to the company's repository
VERBOSITY = 4
...
class Queue_AI(CustomS3):  # AI on server
    def __init__(self, username=USERNAME, send='ai', receive='user', endpoint=ENDPOINT, secret_key_path=SECRET_KEY_PATH,
                 experiment=EXPERIMENT, name='aiS3', save_dir='./', verbosity=VERBOSITY, **kwargs):
        super().__init__(username=username, send=send, receive=receive, endpoint=endpoint, secret_key_path=secret_key_path,
                         experiment=experiment, name=name, save_dir=save_dir, verbosity=verbosity, **kwargs)


class Queue_user(CustomS3):  # user at beamline
    def __init__(self, username=USERNAME, send='user', receive='ai', endpoint=ENDPOINT, secret_key=SECRET_KEY_PATH,
                 experiment=EXPERIMENT, name='userS3', save_dir='./', verbosity=VERBOSITY, **kwargs):
        super().__init__(username=username, send=send, receive=receive, endpoint=endpoint, secret_key=secret_key,
                         experiment=experiment, name=name, save_dir=save_dir, verbosity=verbosity, **kwargs)
...
class MultiClientQueue_user(CustomS3):
    ...
    def __init__(self, client_id=None, username=USERNAME, endpoint=ENDPOINT, secret_key=SECRET_KEY_PATH,
        ...
        super().__init__(username=username, send=send_queue, receive=receive_queue, endpoint=endpoint,
                        secret_key=secret_key, experiment=experiment, name=name, save_dir=save_dir,
