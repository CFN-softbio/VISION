...
with st.sidebar:
    st.session_state.rag_mode = st.radio(
        "Select RAG Mode:",
        ["Naive RAG + PaperQA", "Only Naive RAG"]
    )
...
if prompt := st.chat_input("Ask LLM"):
    ...
    with st.spinner("Thinking ..."):
        ...
        force_reload = False
        PAPER_DIR = "/home2/common/Chatbot_Papers/chemical_safety"
        PERSIST_DIR_NAIVE = "/home2/common/chroma_vectordb_dir/chemical_safety"

        if st.session_state.rag_mode == "Only Naive RAG":
            # Original workflow
