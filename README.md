...
## EnvTrace
To see an implementation of the EnvTrace method (written before the creation of the package), please see [`backend/tests/test_op_cog.py`](backend/tests/test_op_cog.py).
...
The experiments generated by evaluating LLMs on the VISION datasets ([`backend/tests/datasets/op_cog_dataset.json`](backend/tests/datasets/op_cog_dataset.json) and [`backend/tests/datasets/archived/original_op_cog_dataset.json`](backend/tests/datasets/archived/original_op_cog_dataset.json)) using EnvTrace are uploaded on https://zenodo.org/records/17526264 (10.5281/zenodo.17526264).
These results are analyzed and described in the paper "EnvTrace: Simulation-Based Semantic Evaluation of LLM Code via Execution Trace Alignment - Demonstrated at Synchrotron Beamlines" (pending release).
